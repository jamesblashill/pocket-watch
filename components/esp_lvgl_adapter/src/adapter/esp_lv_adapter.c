...
esp_err_t esp_lv_adapter_deinit(void)
{
    ...
    /* Detach LVGL's tick source; a fresh lv_init() on re-entry gets its own
     * default internal tick_state, so nothing else to clean up here. */
    lv_tick_set_cb(NULL);
...
/**
 * @brief LVGL tick-get callback
 *
 * Queried on-demand by lv_tick_get() instead of being pushed by a periodic
 * esp_timer. Removing the periodic timer lets automatic light sleep run for
 * as long as the next *real* event (touch/button GPIO, alarm-check timer,
 * etc.) allows, rather than being capped at the old tick period every cycle.
 */
...
/**
 * @brief Initialize LVGL tick source
 *
 * Points LVGL at esp_timer's free-running microsecond counter instead of
 * incrementing an internal counter from a periodic timer.
 */
static esp_err_t tick_init(void)
{
